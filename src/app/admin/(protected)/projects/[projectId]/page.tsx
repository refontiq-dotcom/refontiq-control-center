...
import { getProjectIntegrations } from "@/lib/project-integrations";
...
export default function ProjectCockpitPage() {
  ...
  const [rotatingIntegrationId, setRotatingIntegrationId] = useState<string | null>(null);
  const [newIntegrationKey, setNewIntegrationKey] = useState<{ integrationId: string; apiKey: string } | null>(null);
  const [integrationKeyError, setIntegrationKeyError] = useState<string | null>(null);

  const rotateIntegrationKey = useCallback(async (integrationId: string) => {
    if (rotatingIntegrationId) return;
    setRotatingIntegrationId(integrationId);
    setNewIntegrationKey(null);
    setIntegrationKeyError(null);

    try {
      const response = await fetch(`/api/admin/projects/${project?.id}/integrations/${integrationId}/credentials`, {
        method: "POST",
        cache: "no-store",
      });
      ...
      if (!response.ok) {
        throw new Error(body?.error || "Impossible de régénérer la clé d'intégration.");
      }

      if (!body.apiKey) throw new Error("Trouvetou n'a pas retourné de nouvelle clé.");
      setNewIntegrationKey({ integrationId, apiKey: body.apiKey });
    } catch (err) {
      setIntegrationKeyError(err instanceof Error ? err.message : "Impossible de régénérer la clé d'intégration.");
    } finally {
      setRotatingIntegrationId(null);
    }
  }, [project?.id, rotatingIntegrationId]);
        ...
        {integrations.length > 0 && (
          <section className="space-y-3">
            {integrations.map((integration) => {
              const rotating = rotatingIntegrationId === integration.integrationId;
              const displayedKey = newIntegrationKey?.integrationId === integration.integrationId ? newIntegrationKey.apiKey : null;

              return (
                <Card key={integration.integrationId} className="border-slate-200 p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h2 className="font-semibold">Intégration {integration.label}</h2>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-[10px] font-semibold uppercase tracking-wide text-slate-500">
                          <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1">{integration.environment}</span>
                          <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1">Super Admin</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
                    ...
                    <p className="mt-1 text-xs leading-5 text-amber-800">
                      L'ancienne clé cesse immédiatement de fonctionner. La nouvelle clé est affichée une seule fois.
                    </p>
                    <Button
                      type="button"
                      className="mt-3 gap-2"
                      variant="warning"
                      disabled={rotatingIntegrationId !== null}
                      onClick={() => void rotateIntegrationKey(integration.integrationId)}
                    >
                      <KeyRound className="h-4 w-4" />
                      {rotating ? "Génération en cours..." : "Régénérer la clé " + integration.label}
                    </Button>
                  </div>

                  {integrationKeyError && rotatingIntegrationId === null && (
                  ...
                  {displayedKey && (
                    <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                      ...
                      <p className="mt-1 text-xs leading-5 text-emerald-800">
                        Copie-la maintenant dans l'environnement Production de {project.name}. Elle ne sera plus récupérable ici.
                      </p>
                      <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                        ...
                        <Button
                          ...
                          onClick={() => void navigator.clipboard.writeText(displayedKey)}
                        >
